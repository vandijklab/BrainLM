...
class ViTMAEEmbeddings(nn.Module):
    ...
    def random_masking(self, sequence, noise=None):
        ...
        if self.config.train_mode == "auto_encode":
            ...
            len_keep = int(seq_length - noise[0, :].sum())
...
class ViTMAEForPreTraining(ViTMAEPreTrainedModel):
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        if pixel_values is None:
            pixel_values = torch.randn([1, 3, 424, 200])
        ...
        if ext_outputs is None:
            ...
            if self.config.train_mode != "auto_encode":
                ...
                patch_end_idx = np.ceil(image_end_idx / self.config.patch_size).astype(int) * self.config.patch_size
                mask_start_idx = image_end_idx - int(self.config.mask_ratio * pixel_values.shape[3])

                # determine number of tokens to mask
                if mask_start_idx % self.config.patch_size == 0:
                    # if mask_start_idx is divisible by patch_size, then we can mask exactly mask_ratio of tokens
                    num_mask = (patch_end_idx - mask_start_idx) // self.config.patch_size
                else:
                    # if patch_end_idx is not divisible by patch_size, then we need to mask one more token only if predicting last without pixel zeroing
                    offset = 1 if self.config.train_mode == "predict_last" else 0
                    num_mask = (patch_end_idx - mask_start_idx) // self.config.patch_size + offset

                # determine indeces of tokens to mask
                end = patch_end_idx // self.config.patch_size
                start = end - num_mask

                # create noise vector
                patch_num = pixel_values_padded.shape[3] // self.config.patch_size
                noise = torch.zeros(pixel_values.shape[0], patch_num, patch_num, device=pixel_values_padded.device)
                noise[:, :, start:end] = 1
                ...
                if self.config.train_mode == "predict_last_pixel":
                    total_pxls_to_mask = int(self.config.mask_ratio * pixel_values.shape[3])
                    patch_start_idx = start * self.config.patch_size
